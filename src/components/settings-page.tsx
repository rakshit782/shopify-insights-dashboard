...
const InfoRow = ({ icon: Icon, label, value, isLoading }: { icon: any; label: string; value: string | null | undefined, isLoading: boolean }) => (
    <div className="flex items-start gap-3">
       <Icon className="h-5 w-5 text-muted-foreground flex-shrink-0 mt-0.5" />
       <div>
           <p className="text-sm font-medium">{label}</p>
           {isLoading ? <Skeleton className="h-5 w-48 mt-1" /> :
               <p className={cn("text-sm text-muted-foreground font-mono")}>{value || 'N/A'}</p>
           }
       </div>
   </div>
);
    ...
    return (
        <Card>
            ...
            <CardContent className="space-y-4">
                ...
                <InfoRow icon={Key} label="Authentication" value={user?.auth0_id} isLoading={isLoading && !user?.auth0_id} />
            </CardContent>
        </Card>
    );
}
